...
function renderHTML(offers, template, make, cb) {
	...
	if (offers) {
		offers.forEach(function(offer, i) {
			...
			if (offer.payment_t1) {
				OFFERS += offer.finance_expiration ? options.payment_t1_expires.replace('{{payment_t1}}', offer.payment_t1).replace('{{finance_expiration}}', offer.finance_expiration.split('T')[0]) : options.payment_t1.replace('{{payment_t1}}', offer.payment_t1);
			};
			if (offer.payment_t2) {
				OFFERS += offer.finance_expiration ? options.payment_t2_expires.replace('{{payment_t2}}', offer.payment_t2).replace('{{finance_expiration}}', offer.finance_expiration.split('T')[0]) : options.payment_t2.replace('{{payment_t2}}', offer.payment_t2);
			};
			if (offer.lease_payment) {
				if (offer.make == "Nissan" || offer.make == "nissan") {
					offer.lease_sales_price ? offer.lease_sales_price = JSON.stringify(offer.lease_sales_price).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") : null;
					offer.lease_down_payment ? offer.lease_down_payment = JSON.stringify(offer.lease_down_payment).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") : null;
					OFFERS += options.lease_payment_expires_vin.replace('{{lease_payment}}', offer.lease_payment).replace('{{lease_expiration}}', offer.lease_expiration.split('T')[0]).replace('{{term}}', offer.lease_term || '').replace('{{downpayment}}', offer.lease_down_payment || '').replace('{{lease_sales_price}}', (offer.lease_sales_price || '')).replace('{{vin}}', offer.vin || '');
				} else {
					offer.lease_down_payment ? offer.lease_down_payment = JSON.stringify(offer.lease_down_payment).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") : null;
					OFFERS += offer.lease_expiration ? options.lease_payment_expires.replace('{{lease_payment}}', offer.lease_payment).replace('{{lease_expiration}}', offer.lease_expiration.split('T')[0]).replace('{{term}}', offer.lease_term).replace('{{downpayment}}', offer.lease_down_payment) : options.lease_payment.replace('{{lease_payment}}', offer.lease_payment);
				}
			};
			if (offer.price) {
			...
			if (offer.interest_rate) {
				...
				OFFERS += offer.finance_expiration ? options.interest_rate_expires.replace('{{interest_rate}}', offer.interest_rate).replace('{{term}}', offer.interest_rate_t1).replace('{{finance_expiration}}', offer.finance_expiration.split('T')[0]) : options.interest_rate.replace('{{interest_rate}}', offer.interest_rate);
			};
