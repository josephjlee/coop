	...
	if (offers) {
		offers.forEach(function(offer, i) {
			...
			if (offer.custom_img_300x110) {
				OFFERS += options_coop.custom_img.replace('{{image_url}}', offer.custom_img_300x110)
			};
			if (offer.payment_t1) {
				OFFERS += offer.finance_expiration ? options_coop.payment_t1_expires.replace('{{payment_t1}}', offer.payment_t1).replace('{{finance_expiration}}', offer.finance_expiration.split('T')[0]) : options_coop.payment_t1.replace('{{payment_t1}}', offer.payment_t1);
			};
			if (offer.payment_t2) {
				OFFERS += offer.finance_expiration ? options_coop.payment_t2_expires.replace('{{payment_t1}}', offer.payment_t2).replace('{{finance_expiration}}', offer.finance_expiration.split('T')[0]) : options_coop.payment_t2.replace('{{payment_t1}}', offer.payment_t2);
			};
			if (offer.lease_payment) {
				OFFERS += offer.lease_expiration ? options_coop.lease_payment_expires.replace('{{lease_payment}}', offer.lease_payment).replace('{{lease_expiration}}', offer.lease_expiration.split('T')[0]).replace('{{term}}', offer.lease_term).replace('{{downpayment}}', offer.lease_down_payment) : options_coop.lease_payment.replace('{{lease_payment}}', offer.lease_payment);
			};
			if (offer.price) {
				offer.price = JSON.stringify(offer.price).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
				OFFERS += options_coop.price.replace('{{price}}', offer.price);
			};
			if (offer.interest_rate) {
				if (offer.interest_rate == "0.0") {
					offer.interest_rate = "0";
				}
				OFFERS += offer.finance_expiration ? options_coop.interest_rate_expires.replace('{{interest_rate}}', offer.interest_rate).replace('{{term}}', offer.interest_rate_t1).replace('{{finance_expiration}}', offer.finance_expiration.split('T')[0]) : options_coop.interest_rate.replace('{{interest_rate}}', offer.interest_rate);
			};
			if (offer.rebate_name_1) {
				offer.rebate_amount_1 = JSON.stringify(offer.rebate_amount_1).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");

				OFFERS += options_coop.rebate_name_1.replace('{{rebate_name_1}}', offer.rebate_name_1).replace('{{rebate_amount_1}}', offer.rebate_amount_1);
			};
			if (offer.rebate_name_2) {
				offer.rebate_amount_2 = JSON.stringify(offer.rebate_amount_2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
				OFFERS += options_coop.rebate_name_2.replace('{{rebate_name_2}}', offer.rebate_name_2).replace('{{rebate_amount_2}}', offer.rebate_amount_2);
			};
			if (offer.rebate_name_3) {
				offer.rebate_amount_3 = JSON.stringify(offer.rebate_amount_3).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
				OFFERS += options_coop.rebate_name_3.replace('{{rebate_name_3}}', offer.rebate_name_3).replace('{{rebate_amount_3}}', offer.rebate_amount_3);
			};
			if (offer.rebate_name_4) {
				offer.rebate_amount_4 = JSON.stringify(offer.rebate_amount_4).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
				OFFERS += options_coop.rebate_name_4.replace('{{rebate_name_4}}', offer.rebate_name_4).replace('{{rebate_amount_4}}', offer.rebate_amount_4);
			};
...
function getParseProfile(cb) {
	...
	if (user_dt.bw_id && user_dt.bw_id !=="{{USER_ID}}") {
	...
	if (make_models.length > 0) {
		...
		for (var i = 0; i < end; i++) {
			...
			if (specials[m_m]) {
				...
				updateBody(specials[m_m], options_coop.template, options_coop[make]);
				return;
			};
			if (i == end - 1) {
				...
				console.log(tt, make);
				updateBody(specials[special_keys[tt]], options_coop.template, options_coop[make]);
			};
		};
	} else {
		var special_keys = Object.keys(specials);
